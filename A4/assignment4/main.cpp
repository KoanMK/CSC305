...
int main(int, char**){
    ...
    window.add_listener<KeyEvent>([&](const KeyEvent &k){
        ...
        if(k.key == GLFW_KEY_W) {
            cameraPos(0) += 0.1;
        } else if (k.key == GLFW_KEY_A) {
            cameraPos(1) += 0.1;
        } else if (k.key == GLFW_KEY_S) {
            cameraPos(0) -= 0.1;
        } else if (k.key == GLFW_KEY_D) {
            cameraPos(1) -= 0.1;
        } else if (k.key == GLFW_KEY_Q) {
            cameraPos(2) += 0.1;
        } else if (k.key == GLFW_KEY_E) {
            cameraPos(2) -= 0.1;
        }
    });
...
void genTerrainMesh() {
    ...
    for(int j=0; j<n_height; ++j) {
        for(int i=0; i<n_width; ++i) {
            ...
            points.push_back(Vec3(f_width * (i / (float(n_width - 0.5))), f_height * (j / (float(n_width - 0.5))), 0.0f) );
